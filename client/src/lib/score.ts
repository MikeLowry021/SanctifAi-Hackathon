...
export function scoreFromSignals(
  ...
  const has = {
    ...
    substances: !!(signals as any).explicit?.substances?.length, // if you added substances
    blasphemy: !!(signals as any).explicit?.blasphemy?.length,
    selfharm: !!(signals as any).explicit?.selfharm?.length,
    theme: (id: string) => signals.themes?.includes(id),
    claim: (needle: string) => signals.claims?.some((c) => c.includes(needle)),
  };

  // Map rule.id → predicate true/false
  const match: Record<string, boolean> = {
    ...
    // doctrinal/ethical examples (keep your existing logic)
    "false-gospel":
    ...
    // positive themes (bonus)
    worship: has.theme("worship"),
    "repentance-hope": has.theme("repentance-hope"),

    // keep your doctrine pack too:
    "salvation-by-grace": has.claim("salvation by grace") || has.theme("grace"),
    "deity-of-christ":
  ...
  // Start from a neutral 50, add each hit's weight, clamp 0..100
